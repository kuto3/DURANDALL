...
const SlotMachine = forwardRef(({ initialSpinCounts , images , isWin, isBigWin }, ref) => {
    ...
    <div className="flex items-start space-x-4">
        ...
        <div key={reelIndex} className="relative w-56 h-[30rem] bg-gray-800 border-gray-500 overflow-hidden rounded-xl">
          ...
          >
            {Array.from({ length: 32 }).map((_, index) => (
              <div
                key={index}
                className={`text-4xl text-center w-56 h-24 overflow-hidden rounded-xl border-2 border-gray-500`}
              >
      ...
      {!isSpinning  && isWin && (
        <div className="absolute top-1/2 left-100 right-0 transform -translate-y-1/2 flex justify-center w-full">
         ...
         {!isSpinning  && isBigWin && (
        <div className="absolute top-1/2 left-100 right-0 transform -translate-y-1/2 flex justify-center w-full">
          <div className="flex justify-center items-center text-4xl text-yellow-600 font-bold border-4 bg-gray-800 border-yellow-500 animate-borderGlow rounded-xl overflow-hidden h-96 w-full ">
          
            <AnimatedText text="CONGRATULATION YOU WIN : 7999$"/>
          </div>
        </div>
      )}
    </div>
